...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorModulesA = Math.sqrt((x1 + y1) ** 2);
  const vectorModulesB = Math.sqrt((x2 + y2) ** 2);
  const result = scalarProduct / (vectorModulesA * vectorModulesB);
...
function getFibonacciNumber(index) {
  ...
  const result =
    (goldenRatio ** index - (1 - goldenRatio) ** index) / 5 ** (1 / 2);
...
function isPowerOfTwo(num) {
  ...
  return Math.round(powerOfTwo) - powerOfTwo === 0;
}
